...
export class checkImage extends plugin {
    ...
    async checkMessyImage() {
        ...
        if (test === false) {
            ...
            await fetch(`https://yunxiyuan.xyz/api/gaffe/goodjob-img/resources/UNKNOWN/${number}.png`, {
                headers: {
                    'Accept': 'image/png',
                }
            })
                .then(response => {
                    if (!response.ok) {
                        ...
                        this.e.reply(segment.image(`https://yunxiyuan.xyz/api/gaffe/goodjob-img/resources/UNKNOWN/${number}.png`))
                        return true
                    }
                })
                .catch(error => {
    ...
    async checkImage() {
        ...
        if (test === false) {
            logger.debug('[无用插件]未发现安装了本地图库，将尝试使用【云溪院API】返图')
            if (`${role}` == `茄子`) {
                await fetch(`https://yunxiyuan.xyz/api/gaffe/goodjob-img/resources/${role}/${number}.gif`, {
                    headers: {
                        'Accept': 'image/gif',
                    }
                })
                    .then(response => {
                        if (!response.ok) {
                            ...
                            logger.debug('[无用插件]云溪院API访问成功!');
                            this.e.reply(segment.image(`https://yunxiyuan.xyz/api/gaffe/goodjob-img/resources/${role}/${number}.gif`))
                            return true
                        }
                    })
                    .catch(error => {
                ...
                await fetch(`https://yunxiyuan.xyz/api/gaffe/goodjob-img/resources/${role}/${number}.png`, {
                    headers: {
                        'Accept': 'image/png',
                    }
                })
                    .then(response => {
                        if (!response.ok) {
                            ...
                            logger.debug('[无用插件]云溪院API访问成功!');
                            this.e.reply(segment.image(`https://yunxiyuan.xyz/api/gaffe/goodjob-img/resources/${role}/${number}.png`))
                            return true
                        }
                    })
                    .catch(error => {
